...
The static build is healthy, the main generated routes respond locally, and there are no Astro diagnostics. The remaining launch work is mostly content confirmation, production Netlify form verification, visual QA, and final deployment checks.
...
- `netlify.toml` is present with build settings, legacy redirects, security headers, and asset cache headers.
- `robots.txt` is present and points to `https://shambalaholisticandretreatcentre.com/sitemap.xml`.
- `sitemap.xml` is generated by Astro and currently includes 40 canonical URLs.
- The sitemap includes `/benefits/`, `/testimonials/`, and `/sound-baths-yoga/`.
- The sitemap intentionally excludes redirected `/workshops/`.
...
- [ ] Verify Netlify redirects and headers in a deployed preview.
  `netlify.toml` is configured, but redirects and headers are applied by Netlify rather than plain Astro preview.
...
- [ ] Decide whether the source `/workshops/` page should remain in the repo.
  Netlify now redirects `/workshops/` to `/sound-baths-yoga/` to match the launch URL plan, and internal links have been moved away from `/workshops/`.
...
- [x] Add `netlify.toml` with build command `npm run build`, publish directory `dist`, redirects, and headers.
